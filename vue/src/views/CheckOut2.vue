              ...
              <div>
                ...
                >
                  ...
                  <span v-if="type == 1" class="fw-bold text-success pe-4"
                    >{{ package?.Price_monthly}} LE</span
                  >
                  <span v-if="type == 2" class="fw-bold text-success pe-4"
                    >{{ package?.Price_annually }} LE</span
                  >
                </label>
  ...
  methods : {
    getSinglrPackage () {
      ...
      axios.get(`${extrnalUrl}packages/${this.route.params.packageId}`, {
        ...
        this.package.user = localStorage.getItem('authUser') ? JSON.parse(localStorage.getItem('authUser')) :  null;
        this.valueYear = this.package.price_monthly;
        this.valueMonth = this.package.Price_monthly;

        console.log("single package", this.package.user)
      })
    },

    checkStripeFields(){
        if (this.subDomain === '') {
          this.domainError = 'ackage.Price_annuallyFill this field.';
        }
    ...
    handleStripePayment() {
        ...
        this.stripe
          ...
          .then(()=>{
            return axios.post(extrnalUrl + "payment/complete", {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
                params: {
