                ...
                <tr>
                    <td class="border-0 pl-0" width="70%">
                        <h4 class="text-uppercase">
                            <strong>{{ $invoice?->name?? "guest"}}</strong>
                        </h4>
                    </td>
                    ...
                    <td class="px-0">
                        @if($invoice->seller->address)
                            <p class="seller-address">
                                <strong> From : </strong> {{ $invoice->seller->address }}
                            </p>
                        @endif

                        @if($invoice->seller->phone)
                            <p class="seller-phone">
                               <strong>  A.B.N : </strong> {{ $invoice->seller->phone }}
                            </p>
                        @endif

                        @if($invoice->seller->name)
                            <p class="seller-phone">
                              <strong> Supplier  : </strong>{{ $invoice->seller->name }}
                            </p>
                        @endif
                            {{--
                    ...
                    <td class="px-0">
                        ...
                        @if($invoice->buyer->address)
                            <p class="buyer-address">
                                <strong> To : </strong> {{ $invoice->buyer->address }}
                            </p>
                        @endif

                        @if($invoice->buyer->phone)
                            <p class="buyer-phone">
                                 <strong> A.B.N  : </strong>{{ $invoice->buyer->phone }}
                            </p>
                        @endif

                        @if($invoice->buyer->name)
                            <p class="buyer-name">
                                <strong> Recipient  : </strong>{{ $invoice->buyer->name }}
                            </p>
                        @endif
        ...
        <table class="table table-items">
            <thead>
                <tr>
                    ...
                    <th scope="col" class="text-right border-0">G.S.T</th>
...
{{--                @if($invoice->hasItemOrInvoiceTax())--}}
                    <tr>
                        <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
                        <td class="text-right pl-0">Total G.S.T</td>
                        <td class="text-right pr-0">
                            @if($invoice->total_taxes != null)
...
{{--                    }--}}
{{--                } else {--}}
{{--                    console.error('Printing is not supported.');--}}
{{--                }--}}
{{--            };--}}
{{--        </script>--}}
        <script type="text/javascript">
            ...
            function printInvoice() {
                ...
                setTimeout(function() {
                    window.print();
                }, 1000); // Adjust this delay as needed to ensure the PDF is fully loaded
            }

            // Call the print function when the document is ready
            window.onload = printInvoice;
        </script>
        <embed src="{{ asset('storage/invoices/'.$invoice->filename)}}" type="application/pdf" style="width:100%;height:100%;" />
    </body>
</html>
