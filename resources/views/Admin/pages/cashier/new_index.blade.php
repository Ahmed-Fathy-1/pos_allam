    ...
    <div class="content">
        ...
        <div class="d-flex flex-column flex-column-reverse flex-lg-row mt-1">
            <div class="col-lg-8 col-sm-12 tabs_wrapper">
                ...
                <!-- categories -->
                <ul class="tabs owl-carousel owl-theme owl-product border-0 p-0">
                    ...
                    @foreach($categories as $category)
                        <li id="{{$category->name.$category->id}}">
                            <div class="product-details d-flex flex-column align-items-center" style="background-color: whitesmoke">
                                @foreach($category->images as $image)
                                    @if($loop->first)
                                        <img class="img-fluid rounded" src="{{ asset('storage/' . $image) }}" alt="img">
                                    @endif
                                @endforeach
                                            ...
                                            <div class="productset flex-fill active">
                                                ...
                                                <div class="productsetcontent">
                                                    ...
                                                    <div class="d-flex justify-content-between gap-2 flex-nowrap add-to-cart-button">
                                                        ...
                                                        <input type="text" class="form-control quantity" placeholder="weight">
                                                    </div>
                                        ...
                                        <div class="productset flex-fill active">
                                            ...
                                            <div class="productsetcontent">
                                                ...
                                                <div class="d-flex justify-content-between gap-2 flex-nowrap add-to-cart-button">
                                                    ...
                                                    <input type="text" class="form-control quantity text-center"  placeholder="weight" required>
                                                </div>
                                                <div style="margin-top:10px; width: 100%">
                                                    <button class="btn btn-primary addToCart d-block w-100" data-product="{{$product->id}}">Add</button>
                                                </div>
            ...
            <div class="col-lg-4 col-sm-12 ">
                <div class="card card-order pt-3">
                    <div class="card-body px-3" style="padding: 0">
                        {{-- add customers --}}
                        <div class="row">
                            <div class="col-12 page-btn">
                               {{-- <a class="btn btn-added w-100 mb-2 text-white" style="background: #ff9f43" data-bs-toggle="modal" data-bs-target="#create">
                                    Add New Customer
                                </a>--}}
                                {{-- create Customer --}}
                                            ...
                                            <form id="customerForm" action="{{ route('newIndex') }}" method="GET">
                                                <select  class="js-example select2" name="customer" required  id="customerSelect">
                                                    ...
                                                    @foreach($customers as $customer)
                                                        <option value="{{$customer->id}}" @if(isset($mobile) && $mobile !=null) @if($mobile->mobile ==$customer->mobile ) selected @endif @endif >{{$customer->name. '(' . $customer->mobile . ")"}}</option>
                                                    @endforeach
                                                </select>
                                            </form>
                                        </div>
                                            ...
                                            <select class="select form-select" name="address_id" id="address_id" form="submit_form">
                                                ...
                                                @if(isset($addresses) && $addresses !=null)
                                                    @foreach($addresses as $address)
                                                         <option value="{{$address->id}}" style=" width: 447px !important" >{{$address->address. ', ' . $address->city . ', ' . $address->state . ' ' . $address->post_code}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                    ...
                    <div id="cartdetails">
                        @php
                            $cart = \App\Models\Cart::where('user_id',auth()->id())->with('cartDetails')->first();
                        @endphp
                        @if($cart)
                        <div class="card-body pt-0">
                            <div class="product-table cart-products ">
                                {{-- products on cart added from response ajax--}}
                                @foreach($cart->cartDetails()->latest('id')->get() as $data)
                                    ...
                                    <li>
                                        <div class="productimg h-100">
                                            @foreach($data->product->images as $image)
                                                ...
                                                @endif
                                            @endforeach
                                            <div class="productcontet d-flex flex-column justify-content-between align-items-stretch" style="height: 65px !important;">
                                                ...
                                                <div class="productlinkset d-flex align-items-center gap-3">
                                                    <h5 class="m-0">{{$data->unit->name}}</h5>
                                                    <span><span style="color: #919192">WT : </span>{{$data->quantity}}</span>
                                                </div>

                                            </div>
                                        </div>
                                    </li>
                                    <li class="d-flex flex-column justify-content-between align-items-end gap-2 me-2">
                                        <a class="deleteproduct" data-detail="{{$data->id}}" ><img src="{{asset('assets/dashboard/img/icons/delete-2.svg')}}" alt="img"></a>
                                        <div class="increment-decrement">
                                            <div class="input-groups">
                                                <p class="text-danger fw-bold"> <span style="color: #919192">Price :</span>  {{$data->price}}</p>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            ...
                            <div class="setvalue" id="total-cart">
                                <ul>
                                    <li class="total-value border-bottom pb-2">
                                        <h5>Total items </h5>
                                        <h6>{{$cart->cartDetails->count()}}</h6>
                                    </li>

                                    <li class="total-value">
                                        <h5>Total price  </h5>
                                        <h5 class="text-danger">${{$cart->total}}</h5>
                                    </li>
                                </ul>
                            </div>
                            <form method="POST" action="{{route('cashier.checkout')}}" id="submit_form">
                                @csrf
                                <div class="d-flex justify-content-end">
                                    <button class="bt btn-primary w-25 text-center" style="padding: 10px;border: 10px; border-radius: 10px" type="submit"><strong>Check Out</strong></button>
                                </div>
                            </form>
                        </div>
                        @else
                            ...
                            <div class="card-body pt-0 pb-2 checkout_cart">
                                <div class="setvalue" id="total-cart">
                                    <ul>
                                        ...
                                        <li class="total-value">
                                            <h5>Total price  </h5>
                                            <h5 class="text-danger">0000</h5>
                                        </li>
                                    </ul>
                                </div>
                                    <div class="d-flex justify-content-end">
                                        <button class="bt btn-primary w-25 text-center disabled" style="padding: 10px;border: 10px; border-radius: 10px" type="submit"><strong>Check Out</strong></button>
                                    </div>
...
@section('js')
    ...
    @include('Admin.pages.cashier.scripts.add_to_cart')
@endsection
