    ...
    <div class="content">
        <div class="page-header">
            <div class="page-title">
                <h4> Delivery Page </h4>
                <h6>Here You see all orders that need to Delivery</h6>
            </div>
            @if(auth()->user()->hasRole('delivery'))
            <div class="d-flex justify-content-end">
                <a href="{{route('delivery-old-orders')}}"
        ...
        @foreach($orders as $order)
            <div class="card mb-5">
                <div class="d-flex align-items-center flex-column flex-lg-row  gap-3 m-4">
                    ...
                    @if(!auth()->user()->hasRole('delivery'))
                    <div class="w-100" >
                        <form method="POST" action="{{route('assign-delivery')}}">
                            @csrf
                            <input type="hidden" name="id" value="{{$order->id}}">
                            <label class="mb-2">Delivery</label>
                            <div class="d-flex align-items-center gap-2 w-100">
                                <div class="form-group mb-0 w-100">
                                    <select class="select form-select" name="delivery_id" required style="min-width: 10rem !important;">
                                        <option selected disabled>Select Delivery</option>
                                        @foreach($users as $user)
                                            <option value="{{$user->id}}" @if($user->id == $order->delivery_id) selected @endif>{{$user->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('delivery_id')
                                    <p class="text-danger">{{$message}}</p>
